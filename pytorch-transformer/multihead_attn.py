...
class MultiHeadAttention(nn.Module):
    ...
    def forward(
        self, x_q, x_k_v, attn_mask
    ):  # x_q: (batch_size,seq_len,emb_size), x_k_v:  (batch_size,seq_len',emb_size)
        ...
        if self.kv_cache_type == "selfattn":  # decoder的自注意力cache
            ...
            if "Q" in self.kv_cache:
                q = torch.concat(
                    (self.kv_cache["Q"], q), dim=1
                )  # 追加到前一次推理的Q末尾
            if "K" in self.kv_cache:
                k = torch.concat(
                    (self.kv_cache["K"], k), dim=1
                )  # 追加到前一次推理的K末尾
            if "V" in self.kv_cache:
                v = torch.concat(
                    (self.kv_cache["V"], v), dim=1
                )  # 追加到前一次推理的K末尾
        ...
        # 多头兼容
        q = q.view(q.size()[0], q.size()[1], self.head, self.q_k_size).transpose(
            1, 2
        )  # q: (batch_size,head,seq_len,q_k_size)
        k = (
            k.view(k.size()[0], k.size()[1], self.head, self.q_k_size)
            .transpose(1, 2)
            .transpose(2, 3)
        ...
        # 注意力与V相乘
        v = v.view(v.size()[0], v.size()[1], self.head, self.v_size).transpose(
            1, 2
        ...
        return z.reshape(
            z.size()[0], z.size()[1], -1
        )  # z: (batch_size,seq_len,head*v_size)
...
if __name__ == "__main__":
    ...
    attn_mask = torch.zeros(
        (1, de_ids_tensor.size()[0], de_ids_tensor.size()[0])
    )  # batch中每个样本对应1个注意力矩阵
