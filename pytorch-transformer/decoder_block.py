...
class DecoderBlock(nn.Module):
    def __init__(self, emb_size, q_k_size, v_size, f_size, head):
        ...
        self.feedforward = nn.Sequential(
            nn.Linear(emb_size, f_size), nn.ReLU(), nn.Linear(f_size, emb_size)
        )
    ...
    def forward(
        self, x, encoder_z, first_attn_mask, second_attn_mask
    ):  # x: (batch_size,seq_len,emb_size)
        # 第1个多头
        z = self.first_multihead_attn(
            x, x, first_attn_mask
        )  # z: (batch_size,seq_len,head*v_size) , first_attn_mask用于遮盖decoder序列的pad部分,以及避免decoder Q到每个词后面的词
        z = self.z_linear1(z)  # z: (batch_size,seq_len,emb_size)
        output1 = self.addnorm1(z + x)  # x: (batch_size,seq_len,emb_size)

        # 第2个多头
        z = self.second_multihead_attn(
            output1, encoder_z, second_attn_mask
        )  # z: (batch_size,seq_len,head*v_size)   , second_attn_mask用于遮盖encoder序列的pad部分,避免decoder Q到它们
...
if __name__ == "__main__":
    ...
    first_attn_mask = (
        (dec_x_batch == PAD_IDX)
        .unsqueeze(1)
        .expand(dec_x_batch.size()[0], dec_x_batch.size()[1], dec_x_batch.size()[1])
    )  # 目标序列的pad掩码
    first_attn_mask = first_attn_mask | torch.triu(
        torch.ones(dec_x_batch.size()[1], dec_x_batch.size()[1]), diagonal=1
    ).bool().unsqueeze(0).expand(dec_x_batch.size()[0], -1, -1).to(
        DEVICE
    )  # &目标序列的向后看掩码
    ...
    second_attn_mask = (
        (enc_x_batch == PAD_IDX)
        .unsqueeze(1)
        .expand(enc_x_batch.size()[0], dec_x_batch.size()[1], enc_x_batch.size()[1])
    ...
    for i in range(5):
        decoder_blocks.append(
            DecoderBlock(emb_size=128, q_k_size=256, v_size=512, f_size=512, head=8).to(
                DEVICE
            )
        )

    for i in range(5):
        dec_x_emb_batch = decoder_blocks[i](
            dec_x_emb_batch, enc_outputs, first_attn_mask, second_attn_mask
        )
