...
class Decoder(nn.Module):
    def __init__(
        ...
        for _ in range(nblocks):
            self.decoder_blocks.append(
                DecoderBlock(emb_size, q_k_size, v_size, f_size, head)
            )
    ...
    def forward(self, x, encoder_z, encoder_x):  # x: (batch_size,seq_len)
        ...
        second_attn_mask = (
            (encoder_x == PAD_IDX)
            .unsqueeze(1)
            .expand(encoder_x.size()[0], x.size()[1], encoder_x.size()[1])
            .to(DEVICE)
